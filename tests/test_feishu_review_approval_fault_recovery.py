...
def main() -> None:
    run = {
        "id": "review-approval-fault-recovery",
        "app_name": "Xrimo",
        "chat_id": "test-chat",
    ...
    with (
        ...
        record_decision_by_run_id(
            run["id"], "submit_review", "card_button", "approver", "review-request-a"
        )
        ...
        create_or_update_fault(
            run_id=run["id"],
            chat_id="test-chat",
            stage="utm-24:expedited-review",
            fault="expedited review result unclear",
            suggested_action="recheck the same page",
        ...
        with (
            patch(
                ...
                return_value={"data": {"message_id": "om_success_after_fault"}},
            ) as send_card,
            redirect_stdout(StringIO()),
        ):
            assert notify_review_success(args, config) == 0

    assert run["review_submission_approval"]["decision"] == "submit_review"
    assert run["review_submission_approval"]["decision_id"] == "review-request-a"
    assert send_card.call_count == 1
    assert send_card.call_args.kwargs["message_uuid"] == run["review_success"]["message_uuid"]
    sent_content = send_card.call_args.args[1]["body"]["elements"][0]["text"]["content"]
