...
| `COPYRIGHT` | `应用名: ` 中第一个全角或半角左括号前的名称；其后的括号备注忽略 |
| `BUNDLE_ID` | `正式包名: ` |
...
1. 从当前运行上下文取得唯一 `<宿主机名称>` 和 `<应用名>-<vm_name>`；页面标题最后一个 `-` 后必须是四位小写 `<vm_name>`。`应用名: ` 的第一个全角或半角左括号及其后内容是备注，生成器只取该括号前的非空名称；此名称必须与标题中的应用名完全一致。
2. 每次读取前先执行父页面校验：
